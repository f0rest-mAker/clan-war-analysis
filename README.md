...
Проект предназначен для автоматизированного сбора, обработки и хранения статистики клановых войн Clash of Clans. Система использует **Apache Airflow** для оркестрации процессов, **PostgreSQL** в качестве целевого хранилища данных и **Apache Superset** для визуализации данных.
...
* **`dds.fact_wars`**: Основная таблица с результатами войн. Содержит общую информацию про клановую войну: размер команд, индикаторы союзного и вражеского клана, флаги результата (`is_allay_winner`, `is_draw`).
* **`dds.fact_clan_stats_at_war`**: Таблица результатами участия кланов в клановых войнах, содержит показатели `stars`, `attacks`, `destruction_percentage`.
...
## Структура расположения файлов для airflow
```
airflow  
├── config  
│   └── airflow.cfg  
